...
def analyzer_ui(TIINGO_TOKEN):
    ...
    if run_analysis:
        ...
        try:
            ...
            # Add resistance lines (above current price)
            for r_detail in sr_levels.get("resistance_details", []):
                strength_color = "#dc2626" if r_detail["strength"] >= 80 else "#f97316" if r_detail["strength"] >= 60 else "#fbbf24"
                line_width = 2 if r_detail["strength"] >= 80 else 1.5

                fig.add_hline(
                    y=r_detail["price"],
                    line_dash="solid",
                    line_color=strength_color,
                    line_width=line_width,
                    opacity=0.7,
                    annotation_text=f"R: ${r_detail['price']:.2f}",
                    annotation_position="left",
                    row=1, col=1
                )

            # Add support lines (below current price)
            for s_detail in sr_levels.get("support_details", []):
                strength_color = "#16a34a" if s_detail["strength"] >= 80 else "#22c55e" if s_detail["strength"] >= 60 else "#86efac"
                line_width = 2 if s_detail["strength"] >= 80 else 1.5
                ...
                fig.add_hline(
                    ...
                    line_width=line_width,
                    opacity=0.7,
                    annotation_text=f"S: ${s_detail['price']:.2f}",
                    annotation_position="left",
                    row=1, col=1
                )
        ...
        try:
            sr_levels = find_support_resistance(df, window=10, num_levels=3)
            current_price = sr_levels.get("current_price", float(df["Close"].iloc[-1]))
            ...
            with sr_col1:
                ...
                if sr_levels.get("price_near_level"):
                    st.caption("⚠️ **Near Key Level**")
                else:
                    st.caption("✅ Clear of levels")

            with sr_col2:
                nearest_resistance = sr_levels.get("nearest_resistance")
                if nearest_resistance:
                    r_price = nearest_resistance["price"]
                    distance = ((r_price - current_price) / current_price) * 100
                    st.metric("Nearest Resistance", f"${r_price:.2f}",
                             delta=f"+{distance:.1f}%", delta_color="off")
                    strength_emoji = "🔴" if nearest_resistance["strength"] >= 80 else "🟠" if nearest_resistance["strength"] >= 60 else "🟡"
                    st.caption(f"{strength_emoji} Strength: {nearest_resistance['strength']}/100")
                else:
            ...
            with sr_col3:
                ...
                if nearest_support:
                    s_price = nearest_support["price"]
                    distance = ((current_price - s_price) / current_price) * 100
                    st.metric("Nearest Support", f"${s_price:.2f}",
                             delta=f"-{distance:.1f}%", delta_color="off")
                    strength_emoji = "🟢" if nearest_support["strength"] >= 80 else "🟢" if nearest_support["strength"] >= 60 else "🟡"
                    st.caption(f"{strength_emoji} Strength: {nearest_support['strength']}/100")
                else:
                    st.metric("Nearest Support", "None detected")

            # Display detailed levels in two columns
            st.divider()
            level_col1, level_col2 = st.columns(2)

            with level_col1:
                st.markdown("### 🔴 Resistance Levels")
                if sr_levels.get("resistance_details"):
                    for i, r in enumerate(sr_levels["resistance_details"], 1):
                        strength_color = "#dc2626" if r["strength"] >= 80 else "#f97316" if r["strength"] >= 60 else "#fbbf24"
                        strength_label = "Strong" if r["strength"] >= 80 else "Moderate" if r["strength"] >= 60 else "Weak"

                        st.markdown(
                            f"<div style='background-color:{strength_color};padding:10px;border-radius:8px;color:white;margin-bottom:8px;'>"
                            f"<b>R{i}: ${r['price']:.2f}</b> (+{r['distance_pct']:.1f}%)<br>"
                            f"<small>Touches: {r['touches']} | Strength: {strength_label} ({r['strength']}/100)</small>"
                            f"</div>",
            ...
            with level_col2:
                ...
                if sr_levels.get("support_details"):
                    for i, s in enumerate(sr_levels["support_details"], 1):
                        strength_color = "#16a34a" if s["strength"] >= 80 else "#22c55e" if s["strength"] >= 60 else "#86efac"
                        strength_label = "Strong" if s["strength"] >= 80 else "Moderate" if s["strength"] >= 60 else "Weak"

                        st.markdown(
                            f"<div style='background-color:{strength_color};padding:10px;border-radius:8px;color:white;margin-bottom:8px;'>"
                            f"<b>S{i}: ${s['price']:.2f}</b> (-{s['distance_pct']:.1f}%)<br>"
                            f"<small>Touches: {s['touches']} | Strength: {strength_label} ({s['strength']}/100)</small>"
                            f"</div>",
                            unsafe_allow_html=True
                        )
                else:
            ...
            st.divider()
            st.markdown("### 💡 S/R Trading Insights")

            insights = []

            # Check if near resistance
            if nearest_resistance and nearest_resistance["price"]:
                distance_to_r = ((nearest_resistance["price"] - current_price) / current_price) * 100
                if distance_to_r < 2:
                    insights.append(f"⚠️ **Near Resistance**: Price is within 2% of resistance at ${nearest_resistance['price']:.2f}. Consider taking profits or waiting for breakout.")
                elif distance_to_r < 5:
                    insights.append(f"📊 **Approaching Resistance**: Resistance at ${nearest_resistance['price']:.2f} is {distance_to_r:.1f}% away. Watch for rejection or breakout.")

            # Check if near support
            if nearest_support and nearest_support["price"]:
                distance_to_s = ((current_price - nearest_support["price"]) / current_price) * 100
                if distance_to_s < 2:
                    insights.append(f"✅ **Near Support**: Price is within 2% of support at ${nearest_support['price']:.2f}. Potential bounce opportunity.")
                elif distance_to_s < 5:
                    insights.append(f"📊 **Approaching Support**: Support at ${nearest_support['price']:.2f} is {distance_to_s:.1f}% away. Watch for bounce or breakdown.")
            ...
            if nearest_support and nearest_resistance:
                ...
                if risk > 0:
                    ...
                    if rr_ratio >= 2:
                        insights.append(f"🎯 **Good Risk/Reward**: R:R ratio is {rr_ratio:.1f}:1 (Risk: ${risk:.2f}, Reward: ${reward:.2f})")
                    elif rr_ratio >= 1:
                        insights.append(f"⚖️ **Moderate Risk/Reward**: R:R ratio is {rr_ratio:.1f}:1 (Risk: ${risk:.2f}, Reward: ${reward:.2f})")
                    else:
                        insights.append(f"⚠️ **Poor Risk/Reward**: R:R ratio is {rr_ratio:.1f}:1 (Risk: ${risk:.2f}, Reward: ${reward:.2f}). Consider waiting for better setup.")

            if insights:
                for insight in insights:
                    st.markdown(insight)
            else:
                st.info("Price is in neutral zone. Monitor for approach to key levels.")

        except Exception as e:
            st.warning(f"Could not display S/R analysis: {e}")
            import traceback
...
- Optimal Entry Price: ${optimal_entry:.2f}
- Swing High (0%): ${fib_levels['0%']:.2f}
- 38.2% Fib: ${fib_levels['38.2%']:.2f}
- 50% Fib: ${fib_levels['50%']:.2f}
- 61.8% Fib: ${fib_levels['61.8%']:.2f}
...
- EMA20: ${current_ema20:.2f} | EMA50: ${current_ema50:.2f}
- Trend: {"UPTREND ✅" if current_ema20 > current_ema50 else "DOWNTREND ⚠️"} (EMA20 {">" if current_ema20 > current_ema50 else "<"} EMA50)
- RSI(14): {current_rsi:.1f} ({"Overbought" if current_rsi > 70 else "Oversold" if current_rsi < 30 else "Neutral"})
- ATR(14): ${current_atr:.2f} (Volatility measure)
- MACD: {current_macd:.2f} | Signal: {current_macd_signal:.2f} | Histogram: {current_macd - current_macd_signal:.2f}
{fib_info}{sr_info}{vol_info}{pattern_info}{earnings_info}{fund_info}
**SETUP TYPE:** {setup_type}

**TRADER'S NOTES:** {st.session_state.get('planner_notes', 'None provided')}
...
Coach me on timing, confirmation, and risk management."""
   ...
   # --- Smarter Auto-Detection Rules (v3: realistic trading behavior) ---
    ...
    with col_btn2:
        # GPT Export for Trade Plan (Enhanced with full technical data)
        with st.popover("💬 Copy for GPT", use_container_width=True):
            # Get comprehensive data for GPT export
            last_row = df.iloc[-1]

            # Get Fibonacci data
            fib_data = calculate_fibonacci_levels(df, lookback=20)
            fib_position = fib_data["current_fib_position"] if fib_data else "N/A"
            fib_zone = fib_data["zone"] if fib_data else "N/A"

            # Get S/R levels
            sr_levels = find_support_resistance(df, window=10, num_levels=2)
            support_str = f"${sr_levels['support'][0]:.2f}" if sr_levels['support'] else "N/A"
            resistance_str = f"${sr_levels['resistance'][0]:.2f}" if sr_levels['resistance'] else "N/A"
            ...
            try:
                patterns = detect_patterns(df)
                pattern_str = ", ".join([f"{p['pattern']}" for p in patterns[:2]]) if patterns else "None detected"
            except:
            ...
            trade_data = {
                ...
                "target": float(target),
                "shares": float(shares),
                "setup_type": entry_signal,
                "timeframe": "Daily",
                "notes": f"Setup: {entry_signal} | Key Level: ${key_level:.2f}",
                # Enhanced data
                "current_price": float(last_row["Close"]),
                "rsi": float(last_row["RSI14"]),
                "ema20": float(last_row["EMA20"]),
                ...
                "rel_volume": vol_analysis.get("relative_volume", 1.0),
