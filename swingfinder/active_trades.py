...
def active_trades_ui() -> None:
    ...
    if st.session_state.get("show_close_modal") and "just_closed_trade" in st.session_state:
        ...
        # Show modal dialog with GPT export (Enhanced)
        @st.dialog("📝 Trade Closed - Copy to GPT Journal")
        def show_gpt_export():
            st.success(f"✅ Trade closed: **{closed_trade.get('symbol')}**")
            st.markdown("---")
            st.markdown("### 💬 Copy this comprehensive review to your GPT:")

            review_prompt = build_trade_review_for_gpt(closed_trade)
            st.text_area(
                "Trade Review (Select all and copy):",
                value=review_prompt,
                height=500,
                key="modal_review_area"
            )

            st.info("💡 **Tip:** Fill out the self-reflection section before pasting into your GPT for better coaching!")

            col1, col2 = st.columns(2)
            with col1:
                if st.button("📋 Select All", use_container_width=True, type="primary"):
                    st.toast("✅ Click in the text area above, then Ctrl+A to select all and Ctrl+C to copy!")
            with col2:
