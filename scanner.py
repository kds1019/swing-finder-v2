...
def scanner_ui(TIINGO_TOKEN):
    ...
    def evaluate_ticker(ticker: str, mode: str, price_min: float, price_max: float, min_volume: float) -> dict | None:
        ...
        try:
            ...
            # Apply Fibonacci weighting to Smart Score
            if fib_data:
                fib_position = fib_data["current_fib_position"]
                fib_zone = fib_data["zone"]

                # Boost score for discount zone entries (better risk/reward)
                if fib_zone == "discount":
                    if fib_position <= 38.2:
                        smart_score += 15  # Deep discount - excellent entry
                    elif fib_position <= 50:
                        smart_score += 10  # Discount zone - good entry

                # Lower score for premium zone entries (worse risk/reward)
                elif fib_zone == "premium":
                    if fib_position >= 78.6:
                        smart_score -= 15  # Extended premium - risky entry
                    elif fib_position >= 61.8:
                        smart_score -= 10  # Premium zone - less favorable

                # Clamp again after Fibonacci adjustment
            ...
            card = {
                ...
                # Fibonacci data
                "FibPosition": round(fib_data["current_fib_position"], 1) if fib_data else None,
                "FibZone": fib_data["zone"] if fib_data else None,
                "FibZoneLabel": get_fibonacci_zone_label(fib_data["current_fib_position"]) if fib_data else None,
                "OptimalEntry": round(fib_data["optimal_entry"], 2) if fib_data else None,
                "FibLevels": fib_data["fib_levels"] if fib_data else None,
                "SwingHigh": round(fib_data["swing_high"], 2) if fib_data else None,
                "SwingLow": round(fib_data["swing_low"], 2) if fib_data else None,
    ...
    def run_full_scan(mode: str, price_min: float, price_max: float, min_volume: float,
        ...
        # ✅ DEBUG: Show all qualified tickers to verify variety
        if unique_results:
            all_symbols = [r.get("Symbol") for r in unique_results]
    ...
    if not results:
        ...
        # ✅ Show SmartScore distribution summary
        if confirmed:
            scores = [r.get("SmartScore", 0) for r in confirmed]
            avg_score = sum(scores) / len(scores) if scores else 0
            max_score = max(scores) if scores else 0
            min_score = min(scores) if scores else 0

            # Count score ranges for variety check
            high_scores = len([s for s in scores if s >= 70])
            mid_scores = len([s for s in scores if 50 <= s < 70])
            low_scores = len([s for s in scores if s < 50])

            st.info(f"📊 **SmartScore Summary:** Avg: {avg_score:.1f} | Range: {min_score:.0f}-{max_score:.0f} | "
        ...
        if st.session_state.get("fib_filter", False):
            ...
            filtered_count = (original_confirmed_count - len(confirmed)) + (original_near_count - len(near_misses))
            if filtered_count > 0:
                st.info(f"💎 Fibonacci Filter: Showing only discount zone entries. Filtered out {filtered_count} premium zone stocks.")
        ...
        if st.session_state.get("earnings_filter", False):
            ...
            near_misses = [r for r in near_misses if not (r.get('DaysToEarnings') is not None and r.get('DaysToEarnings') <= 7)]

            filtered_count = (original_confirmed_count - len(confirmed)) + (original_near_count - len(near_misses))
            if filtered_count > 0:
                st.info(f"⚠️ Earnings Filter: Excluded {filtered_count} stocks with earnings in next 7 days.")

        # ✅ Apply Sector filter if enabled (only show hot sectors)
        if st.session_state.get("sector_filter", False) and st.session_state.get("smart_mode", False):
            original_confirmed_count = len(confirmed)
            original_near_count = len(near_misses)
            ...
            try:
                rotation = analyze_sector_rotation(TIINGO_TOKEN)
                hot_sectors = [s['sector'] for s in rotation.get('hot_sectors', [])]

                if hot_sectors:
                    # Filter to only show stocks in hot sectors
                    confirmed = [r for r in confirmed if any(hot_sec.lower() in r.get('Sector', '').lower() for hot_sec in hot_sectors)]
                    near_misses = [r for r in near_misses if any(hot_sec.lower() in r.get('Sector', '').lower() for hot_sec in hot_sectors)]
                ...
                for r in range(rows):
                    ...
                    for j, col in enumerate(cols):
                        ...
                        with col:
                            ...
                            fib_badge = ""
                            if rec.get('FibZone'):
                                if rec['FibZone'] == 'discount':
                                    fib_badge = f"💎 Discount Entry ({rec['FibPosition']:.0f}% Fib)"
                                else:
                                    fib_badge = f"⚠️ Premium Zone ({rec['FibPosition']:.0f}% Fib)"

                            # Build earnings badge
                            earnings_badge = ""
                            if rec.get('EarningsWarning'):
                                earnings_badge = rec['EarningsWarning']
                            ...
                            if rec.get('Sector') and rec['Sector'] != 'Unknown':
                                ...
                                if st.session_state.get("smart_mode", False):
                                    try:
                                        rotation = analyze_sector_rotation(TIINGO_TOKEN)
                                        hot_sectors = [s['sector'] for s in rotation.get('hot_sectors', [])]
                                        is_hot = any(hot_sec.lower() in rec['Sector'].lower() for hot_sec in hot_sectors)
                                    except:
                                        pass
        ...
        with st.expander("🎯 Watchlist Screener Results", expanded=True):
            ...
            for r in range(rows):
                ...
                for j, col in enumerate(cols):
                    ...
                    with col:
                        # Show fund badge only if score exists
                        has_fund_score = rec.get('FundScore') and rec.get('FundScore') > 0
